...
- **모델 개선**: 보다 정교한 견고성 기술(특히 노이즈 / 데이터 손상에 대한 견고함) 도입 또는 신경망 추천 시스템 적용. 특히 넷플릭스 유저 데이터 확장 시 고차원 데이터로 확장되며 GMM은 이 부분에서 한계를 보입니다. 따라서 데이터 전처리 단계에서 차원 축소를 진행하거나 신규 모델 개발을 통해 성능 일관성을 확보해야 합니다.
